...
```python
from shoghi.commercial import CommercialAgentBuilder, ProductCategory
...
```python
from shoghi.commercial import CommercialAgentRegistry
...
```python
from shoghi.commercial import CommercialFeeProcessor
...
```python
from shoghi.commercial import CommercialTransparencyReport
...
```python
from shoghi.commercial.examples import create_aloha_meals_agent
...
Run the example:
```bash
python -m shoghi.commercial.examples.meal_delivery_agent
```
...
Run tests:
```bash
pytest shoghi/tests/test_commercial.py -v
```

## Integration with Shoghi

Commercial agents integrate with existing Shoghi infrastructure:
...
Part of the Shoghi project - Servant coordination for Ohana communities.
