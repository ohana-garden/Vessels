"""
Shoghi Graphiti Client

Wrapper around Graphiti for Shoghi-specific functionality:
...
from .schema import (
    ...
    ShorghiGraphSchema, CommunityPrivacy
)

logger = logging.getLogger(__name__)


class ShorghiGraphitiClient:
    """
    Shoghi-specific wrapper around Graphiti/FalkorDB

    Provides:
    - Namespace isolation per community
    - Privacy-filtered queries
    - Temporal validity tracking
    - Simplified API for Shoghi patterns
    """
    ...
    def __init__(
        ...
        self.graph_name = ShorghiGraphSchema.create_community_graph_name(community_id)
    ...
    def create_node(
        ...
        # Validate properties
        if not ShorghiGraphSchema.validate_node_properties(node_type, properties):
            missing = set(ShorghiGraphSchema.get_required_properties(node_type)) - set(properties.keys())
