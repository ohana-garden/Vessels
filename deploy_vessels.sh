...
deploy_platform() {
    ...
    print_status "Deploying Vessels Platform in $mode mode..."
...
deploy_development() {
    print_status "Starting Vessels Platform in development mode..."
    
    # Run the platform
    print_status "Starting interactive mode..."
    python3 vessels.py --mode development
}

# Production deployment
deploy_production() {
    print_status "Starting Vessels Platform in production mode..."
    ...
    if [ "$DOCKER_AVAILABLE" = true ]; then
        ...
        if curl -f http://localhost:8080/health >/dev/null 2>&1; then
            print_success "Vessels Platform is healthy and ready!"
        ...
        nohup python3 vessels.py --mode production > vessels.log 2>&1 &
        VESSELS_PID=$!
        echo $VESSELS_PID > vessels.pid
        print_success "Vessels Platform started (PID: $VESSELS_PID)"
    fi
}

# Deployed deployment (cloud/edge)
deploy_deployed() {
    print_status "Deploying Vessels Platform to cloud/edge..."
    ...
    python3 -c "
from auto_deploy import deploy_vessels_platform
print('🚀 Deploying Vessels Platform...')
deployment_id = deploy_vessels_platform('cloud')
...
show_status() {
    print_status "Checking Vessels Platform status..."
    
    if [ -f "vessels.pid" ]; then
        local pid=$(cat vessels.pid)
        if ps -p $pid > /dev/null 2>&1; then
            print_success "Vessels Platform is running (PID: $pid)"
        else
            print_warning "Vessels Platform is not running (stale PID file)"
            rm -f vessels.pid
        fi
    else
        print_warning "Vessels Platform is not running"
    fi
    
    # Show detailed status if platform is available
    if python3 -c "import sys; sys.path.insert(0, '.'); from vessels import VesselsPlatform; print('Platform available')" 2>/dev/null; then
        print_status "Detailed platform status:"
        python3 vessels.py --status
    fi
}

# Stop platform
stop_platform() {
    print_status "Stopping Vessels Platform..."
    ...
    if [ -f "vessels.pid" ]; then
        ...
        if ps -p $pid > /dev/null 2>&1; then
            ...
            kill $pid
            rm -f vessels.pid
            print_success "Vessels Platform stopped"
        else
