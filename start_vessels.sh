...
echo "🌺 VESSELS - Voice-First Community Coordination Platform"
echo "======================================================"
echo ""
...
pkill -f "vessels_web_server.py" 2>/dev/null
sleep 1

# Start the web server
echo "🚀 Starting Vessels Web Server..."
nohup python3 vessels_web_server.py > logs/web_server.log 2>&1 &
WEB_PID=$!
...
echo ""
echo "🛑 To stop: Press Ctrl+C or run ./stop_vessels.sh"
echo ""
echo "======================================================"

# Create stop script
cat > stop_vessels.sh << 'EOF'
#!/bin/bash
echo "Stopping Vessels..."
pkill -f "vessels_web_server.py"
echo "✅ Vessels stopped"
EOF
chmod +x stop_vessels.sh

# Keep script running
trap "echo ''; echo 'Stopping Vessels...'; kill $WEB_PID 2>/dev/null; exit 0" INT TERM
